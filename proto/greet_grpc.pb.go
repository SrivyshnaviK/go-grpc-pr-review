...
type GreetServiceClient interface {
	...
	SayBidirectionalStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayBidirectionalStreamingClient, error)
}
...
func (c *greetServiceClient) SayBidirectionalStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayBidirectionalStreamingClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[2], "/greet_service.GreetService/SayBidirectionalStreaming", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServiceSayBidirectionalStreamingClient{stream}
	return x, nil
}

type GreetService_SayBidirectionalStreamingClient interface {
	Send(*HelloRequest) error
	Recv() (*HelloResponse, error)
	grpc.ClientStream
}

type greetServiceSayBidirectionalStreamingClient struct {
	grpc.ClientStream
}

func (x *greetServiceSayBidirectionalStreamingClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greetServiceSayBidirectionalStreamingClient) Recv() (*HelloResponse, error) {
	m := new(HelloResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type GreetServiceServer interface {
	...
	SayBidirectionalStreaming(GreetService_SayBidirectionalStreamingServer) error
...
func (UnimplementedGreetServiceServer) SayBidirectionalStreaming(GreetService_SayBidirectionalStreamingServer) error {
	return status.Errorf(codes.Unimplemented, "method SayBidirectionalStreaming not implemented")
}
...
func _GreetService_SayBidirectionalStreaming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).SayBidirectionalStreaming(&greetServiceSayBidirectionalStreamingServer{stream})
}

type GreetService_SayBidirectionalStreamingServer interface {
	Send(*HelloResponse) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greetServiceSayBidirectionalStreamingServer struct {
	grpc.ServerStream
}

func (x *greetServiceSayBidirectionalStreamingServer) Send(m *HelloResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greetServiceSayBidirectionalStreamingServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SayBidirectionalStreaming",
			Handler:       _GreetService_SayBidirectionalStreaming_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
